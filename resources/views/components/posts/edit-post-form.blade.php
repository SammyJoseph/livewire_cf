<div x-data="{ openEditModal : @entangle('openEditModal') }">
    <!-- Edit post - Modal -->
    <div x-show="openEditModal" x-cloak
        ...
        <form tabindex="-1" aria-hidden="true" @click.outside="openEditModal = false" wire:submit="update" wire:click.away="resetEPost"
            class="w-full p-4 overflow-x-hidden overflow-y-auto max-w-lg bg-white shadow-lg rounded-lg">
            <div class="relative w-full max-w-lg max-h-full">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <button type="button" @click="openEditModal = false" wire:click="resetEPost"
                    ...
                    <div class="px-6 py-6 lg:px-8">
                        ...
                        <form class="space-y-6" action="#">
                            ...
                            <div> {{-- Vista previa de imagen --}}
                                @if (Str::startsWith($image, 'posts/'))
                                    <img src="{{ Storage::url($image) }}" alt="{{ $title }}">
                                @else
                                    <img src="{{ $image->temporaryUrl() }}" alt="{{ $title }}">
                                @endif
                            ...
                            <button type="submit" wire:loading.attr="disabled" wire:target="update, image"
                                ...
                                Actualizar Post
                                <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-device-floppy inline-block -mt-1 w-5" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
